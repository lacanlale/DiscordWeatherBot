...
class CityJSON constructor(city: String,
                           state: String,
                           key: String){
    private val googleGeocodingAPIURL = "https://maps.googleapis.com/maps/api/geocode/json?address=+${city.replace(" ", "+").trim()},+${state.trim()}&key=$key"

    private val connection = URL(googleGeocodingAPIURL).openConnection() as HttpURLConnection
